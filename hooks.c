...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/26 22:59:06 by afadouac          #+#    #+#             */
/*   Updated: 2023/12/26 22:59:10 by afadouac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_hook_julia(int button, int x, int y, t_data *data)
{
	...
	{
		data->c.real = 0.7885 * cos(fabs(complex(x, y, *data).real) * M_PI);
		data->c.imag = 0.7885 * sin(fabs(complex(x, y, *data).real) * M_PI);
		generate_julia_set(*data);
		mlx_put_image_to_window(data->mlx, data->win, data->img, 0, 0);
	}
	return (0);
